...
object SparkPatentClassification {
  ...
  /*  var orgMap: Map[String, Int] = Map()
    ...
    val mlprep = patentsRDD.map(patent => {
      ...
      Array(word1.toDouble, word2.toDouble, word3.toDouble)
    })
    ...
    categoricalFeaturesInfo += (0 -> word2Map.size)
    categoricalFeaturesInfo += (1 -> word3Map.size)

    val numClasses = word1Map.size
